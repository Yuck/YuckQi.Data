...
public class RetrievalHandler<TEntity, TIdentifier, TScope> : RetrievalHandlerBase<TEntity, TIdentifier, TScope?> where TIdentifier : struct, IEquatable<TIdentifier> where TEntity : class, IEntity<TIdentifier> where TScope : DbContext?
{
    ...
    protected override TEntity? DoGet(TIdentifier identifier, TScope? scope)
    {
        ...
        return scope.Find<TEntity>(identifier); // TODO: scope.Set<TEntity>().SingleOrDefault(t => t.Identifier == identifier)
    }
    ...
    protected override async Task<TEntity?> DoGet(TIdentifier identifier, TScope? scope, CancellationToken cancellationToken)
    {
        ...
        return await scope.FindAsync<TEntity>(identifier);
    }
